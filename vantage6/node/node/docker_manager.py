...
class DockerManager(object):
    ...
    def run(self, result_id: int,  image: str, docker_input: bytes,
        ...
        # Work around for overlay container network
        env_file = open("env.list", "w")
        json.dump(environment_variables, env_file)
        env_file.close()
        ...
        try:
            ...
            subprocess.run(["docker", "run", "--env-file", "env.file", "-dit", "--name", 
                "subproces-container", "--network", "over-net", image])
